...
function getCustomPois() {
  try { return JSON.parse(localStorage.getItem(CUSTOM_POIS_KEY)) || []; }
  catch { return []; }
}
...
function getBaseEdits() {
  try { return JSON.parse(localStorage.getItem(BASE_EDITS_KEY)) || {}; }
  catch { return {}; }
}
...
function clearRoute() {
  ...
  activeRoute = null; originPoi = null; navMode = null;
  hideNavBar();
}
...
function hideNavBar() { document.getElementById('nav-bar').classList.remove('open'); }
...
function placeUserMarker(coords) {
  ...
  el.style.cssText = `width:18px;height:18px;border-radius:50%;background:#1A73E8;border:3px solid white;box-shadow:0 0 0 5px rgba(26,115,232,0.2);`;
  userMarker = new mapboxgl.Marker({ element: el, anchor: 'center' }).setLngLat(coords).addTo(map);
}
...
function renderLoginScreen() {
  ...
  overlay.innerHTML = `
    <div class="login-card">
      <div class="login-logo"><img src="./public/img/logo.png" alt="BUAP"></div>
...
function renderTopBar(session) {
  ...
  bar.id = 'user-bar';
  bar.innerHTML = `
    <span class="user-bar-name">${session.role === 'admin' ? '' : ''} ${session.name}</span>
    ${session.role === 'admin' ? `<button id="btn-admin-mode" class="user-bar-btn admin-btn" title="Modo agregar POI">＋ Agregar</button>` : ''}
...
function refreshFavList(panel) {
  ...
  favs.forEach(name => {
    ...
    item.querySelector('.fav-item-remove').addEventListener('click', (e) => {
      ...
      refreshFavList(null);
      // Actualizar botón en el detail panel si está abierto
    ...
    // Al tocar el item, volar al POI
    item.addEventListener('click', (e) => {
      if (e.target.classList.contains('fav-item-remove')) return;
      map.flyTo({ center: poi.coords, zoom: 17, pitch: 45, duration: 800 });
...
function showAddPoiModal(coords) {
  ...
  document.getElementById('poi-cancel-btn').addEventListener('click', () => {
    ...
    const btn = document.getElementById('btn-admin-mode');
    if (btn) { btn.classList.remove('active'); btn.textContent = '＋ POI'; }
    map.getCanvas().style.cursor = '';
  });
  ...
  document.getElementById('poi-save-btn').addEventListener('click', () => {
    ...
    const newPoi = { id: genId(), name, desc: desc || 'Sin descripción.', cat, coords };
    ...
    // Renderizar el nuevo marcador en el mapa
...
function showEditPoiModal(poi) {
  ...
  modal.innerHTML = `
    <div class="add-poi-card">
      ...
      ${poi.isBase ? `<p class="add-poi-coords" style="color:#C9933A">⚠️ POI base — solo se editan nombre, descripción y categoría</p>` : `<p class="add-poi-coords">📍 ${poi.coords[1].toFixed(5)}, ${poi.coords[0].toFixed(5)}</p>`}
      <div class="login-field">
        <label>Nombre</label>
  ...
  document.getElementById('edit-poi-save').addEventListener('click', () => {
    ...
    if (poi.isBase) {
      // Guardar como override en BASE_EDITS
      const edits = getBaseEdits();
      edits[poi.id] = { name: newName, desc: newDesc || poi.desc, cat: newCat };
      saveBaseEdits(edits);
    } else {
      // Actualizar en CUSTOM_POIS
      const customs = getCustomPois();
      const idx = customs.findIndex(p => p.id === poi.id);
      if (idx !== -1) {
        customs[idx] = { ...customs[idx], name: newName, desc: newDesc || poi.desc, cat: newCat };
        saveCustomPois(customs);
      }
    }

    closeModal();
    rebuildMarker(poi.id ?? poi.name, { name: newName, desc: newDesc || poi.desc, cat: newCat });
...
function rebuildMarker(poiId, changes) {
  ...
  const updatedPoi = { ...old.poi, ...changes };
  addMarkerToMap(updatedPoi);

  // Actualizar el panel si estaba mostrando este POI
...
function addMarkerToMap(poi) {
  ...
  // Wrapper que contiene label + pin
  ...
  wrapper.addEventListener('click', () => {
    ...
    // Botones admin: editar siempre visible, eliminar solo en POIs custom
    ...
    if (adminActions) {
      if (session?.role === 'admin') {
        ...
        if (btnDelete) btnDelete.style.display = poi.isBase ? 'none' : 'flex';
      } else {
  ...
  allMarkers.push({ el: wrapper, elPin: el, poi, marker });
  return { el: wrapper, elPin: el, poi, marker };
}
...
function initApp(session) {
  ...
  if (map.loaded()) loadMarkers();
  else map.once('load', loadMarkers);

  // Botón favorito en el panel de detalle
  const favBtn = document.getElementById('panel-fav-btn');
  if (favBtn) {
    favBtn.addEventListener('click', () => {
      ...
      const favPanel = document.getElementById('fav-panel');
      if (favPanel?.classList.contains('open')) refreshFavList(null);
    });
  }
  ...
  // Admin: clic en mapa para agregar POI
  map.on('click', (e) => {
...
map.on('load', () => {
  ...
  map.addSource('campus', { type: 'geojson', data: `${import.meta.env.BASE_URL}data/buap.geojson` });
  map.addLayer({ id: 'campus-ground', type: 'fill', source: 'campus',
    paint: { 'fill-color': '#CBD5E1', 'fill-opacity': 0.55 } });
  map.addLayer({ id: 'campus-border', type: 'line', source: 'campus',
    paint: { 'line-color': '#3A3A3A', 'line-width': 3, 'line-opacity': 0.8 } });
...
document.getElementById('btn-location').addEventListener('click', () => {
  if (!navigator.geolocation) { showNavError('Tu navegador no soporta geolocalización.'); return; }
...
document.getElementById('btn-rotate').onclick  = () => map.rotateTo(map.getBearing() + 90, { duration: 900 });
...
// Paneles que se cierran con Esc / clic fuera
// Orden importa: el primero en la lista tiene mayor prioridad
const DISMISSABLE = [
  { id: 'add-poi-modal',   close: () => { document.getElementById('add-poi-modal')?.classList.remove('open'); adminClickMode = false; const b = document.getElementById('btn-admin-mode'); if (b) { b.classList.remove('active'); b.textContent = '＋ POI'; } map.getCanvas().style.cursor = ''; } },
  { id: 'edit-poi-modal',  close: () => document.getElementById('edit-poi-modal')?.classList.remove('open') },
  { id: 'confirm-modal',   close: () => document.getElementById('confirm-modal')?.classList.remove('open')  },
  { id: 'poi-selector',    close: () => document.getElementById('poi-selector').classList.remove('open')    },
  { id: 'fav-panel',       close: () => document.getElementById('fav-panel')?.classList.remove('open')      },
  { id: 'detail-panel',    close: () => document.getElementById('detail-panel').classList.remove('open')    },
];

// Esc — cierra el panel más prioritario que esté abierto
...
// Clic fuera — cierra si el clic no fue dentro del panel
document.addEventListener('pointerdown', (e) => {
  for (const { id, close } of DISMISSABLE) {
    const el = document.getElementById(id);
    if (!el?.classList.contains('open')) continue;
    // Los modales con fondo oscuro (add/edit/confirm) se cierran al tocar el fondo
    if (['add-poi-modal', 'edit-poi-modal', 'confirm-modal'].includes(id)) {
      if (!el.querySelector('.add-poi-card')?.contains(e.target)) { close(); return; }
    } else {
      // Paneles bottom-sheet: se cierran si el toque fue fuera del panel
